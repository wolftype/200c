...
So if you were manipulation frame properties directly using:
...
Keeping the code to under 1500 lines means it can be read in full.
...
A Quaternion represents a rotation in 3D.  It can be used to transform a [GFX.Vector](#vector).  The following code creates a quaternion that rotates a vector 90 degrees around the y axis:
...
It's values should not be set directly, but rather with methods like `.setAxisAngle` and `.setEuler`.
